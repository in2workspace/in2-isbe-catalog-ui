...
export class CategoryItemComponent implements OnInit {
  ...
  simplifiedId: any;
  labelClass: string =
    'inline-flex items-center justify-between w-full px-5 py-3 text-gray-500 bg-white border-2 rounded-lg cursor-pointer peer-checked:border-primary-50 hover:text-gray-600 peer-checked:text-gray-600 hover:bg-gray-50';
  labelClassParentFirst: string =
    'flex items-center justify-between w-full px-5 py-3 font-medium rtl:text-right text-gray-500 border border-b-0 border-gray-200 rounded-t-xl focus:ring-4 focus:ring-gray-200 hover:bg-gray-100 gap-3 peer-checked:border-primary-50 peer-checked:text-gray-600';
  labelClassParentLast: string =
    'flex items-center justify-between w-full px-5 py-3 font-medium rtl:text-right text-gray-500 border border-gray-200 focus:ring-4 focus:ring-gray-200 hover:bg-gray-100 gap-3 peer-checked:border-primary-50 peer-checked:text-gray-600';
  labelClassParent: string =
    'flex items-center justify-between w-full px-5 py-3 font-medium rtl:text-right text-gray-500 border-t border-r border-l border-gray-200 focus:ring-4 focus:ring-gray-200 hover:bg-gray-100 gap-3 peer-checked:border peer-checked:border-primary-50 peer-checked:text-gray-600';
  ...
  ngOnInit() {
    ...
    if (categories.length > 0) {
      const index = categories.findIndex(
        (item: Category) => item.id === this.data?.id
      );
      if (index > -1) {
        this.checked = true;
      }
  ...
  onClick(checked: boolean) {
    if (!checked) {
      ...
      const index = this.checkedCategories.findIndex(
        (item) => item === (this.data as Category).id
      );
      if (index !== -1) {
        this.checkedCategories.splice(index, 1);
      }
